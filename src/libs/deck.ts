...
export const calculateCardPoints = (card: CardType): number => {
...
export const calculateCardsPoints = (cards: CardType[]): number =>
  cards.reduce((total, card) => total + calculateCardPoints(card), 0)
...
export const findDiscardPickupMeld = (hand: CardType[], pickedCards: CardType[]): CardType[] | null => {
  ...
  const candidates = findMeldCandidates([...hand, ...pickedCards]).filter(
    (candidate) =>
      candidate.some((card) => card.id === requiredCard.id) &&
